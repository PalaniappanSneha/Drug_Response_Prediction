...
def main(args, train_data, valid_data, test_data,train_label, valid_label, test_label):
    ...
    plt.savefig(os.path.join(args.expr_dir, 'testing_x3.png'))

    # return best_valid_loss, test_loss_best_val, avg_test_r_square, std_test_r_square
    return best_valid_loss, test_loss_best_val, avg_test_r_square, std_test_r_square, max_r2_train, max_r2_test, best_train_loss, PCA_TT
...
if __name__ == '__main__':
    ...
    #1) data loading
    if args.data == 'RPPA_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_RPPA_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_RPPA_No_PCA(cv=args.cv)
        args.num_param = 214

    elif args.data == 'Meta_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_Meta_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_Meta_No_PCA(cv=args.cv)
        args.num_param = 225

    elif args.data == 'Mut_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_Mutations_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_Mutations_No_PCA(cv=args.cv)
        args.num_param = 18771

    elif args.data == 'CNV_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_CNV_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_CNV_No_PCA(cv=args.cv)
        args.num_param = 24501

    elif args.data == 'Exp_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_Expression_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_Expression_No_PCA(cv=args.cv)
        args.num_param = 17359

    elif args.data == 'miRNA_No_PCA':
        if args.cv:
            train_val_data, train_val_label, test_data, test_label = read_miRNA_No_PCA(cv=args.cv)
        else:
            train_data, train_label, valid_data, valid_label, test_data, test_label = read_miRNA_No_PCA(cv=args.cv)
        args.num_param = 734
    ...
    if args.cv:
        ...
        for train_index, test_index in kf.split(train_val_data):
            ...
            best_valid_loss, test_loss_best_val, avg_test_r_square, std_test_r_square, max_r2_train, max_r2_test, best_train_loss, PCA_TT = main(args, train_data, val_data, test_data, train_label, val_label, test_label)
        ...
        best_valid_loss, test_loss_best_val, avg_test_r_square, std_test_r_square, max_r2_train, max_r2_test, best_train_loss, PCA_TT = main(args, train_data, valid_data, test_data, train_label, valid_label, test_label)
